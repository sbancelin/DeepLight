...
class SpectroManager(QObject):
    ...
    def __init__(self, hardware_manager=None, parent=None):
        ...
        self.mapping_timer.timeout.connect(self._acquire_next_pixel)
    ...
    def stop_mapping(self):
        self._stop_mapping_internal(emit_finished=True, status_text="Spectro acquisition stopped")
    ...
    def start_live_brillouin(self, params=None):
        ...
        interval_ms = self._compute_brillouin_live_interval_ms(self._brillouin_live_params)
        self.brillouin_live_timer.start(interval_ms)
    ...
    def stop_live_brillouin(self):
        ...
        self.brillouin_live_timer.stop()
        self._brillouin_live_running = False
    ...
    def _on_brillouin_live_timeout(self):
        try:
            ...
            self.stop_live_brillouin()
            self.sigFailed.emit(f"Brillouin live failed: {e}")
    ...
    def _acquire_next_pixel(self):
        ...
        try:
            ...
            x_um, y_um, z_um = float(pos_um[0]), float(pos_um[1]), float(pos_um[2])
            ...
            # cadence mapping = paramètres du panel, pas ceux du widget snap/live
            ...
            delay_ms = max(1, int(round(exposure_ms + settle_ms)))

            self._schedule_next_pixel(delay_ms=delay_ms)
